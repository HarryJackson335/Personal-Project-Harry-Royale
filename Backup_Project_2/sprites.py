...
class Character(pygame.sprite.Sprite):
    def __init__(self, character_type, pos):
        super().__init__()
        
        self.name = self.character_type = character_type
        self.image = CHARACTERS[character_type]['image'].convert_alpha()
        self.image = pygame.transform.flip(self.image, True, False) if character_type == 'player' else self.image
        self.rect = self.image.get_frect(center = pos) if self.name == 'player' else self.image.get_frect(midright = pos)
        ...
        self.direction = 1 if self.name == 'player' else -1
        self.speed = 300
    ...
    def __init__(self):
        ...
        self.surf = self.font.render(
            '''
                ...
                If you hit surrender, the battle ends and you die.
        ...
        self.next_surf = self.font.render('NEXT ->', True, COLORS['white'], COLORS['green'])
